...
def myAI(game):
  ...
  if theCity.getProductionType() == "R":
    if len(otherCitiesDictionary["S"]) != 0:
      findNearestCityAndSend(game, biggestCity, otherCitiesDictionary["S"])
      return
    elif len(otherCitiesDictionary["R"]) != 0:
      findNearestCityAndSend(game, biggestCity, otherCitiesDictionary["R"])
      return
  elif theCity.getProductionType() == "P":
    if len(otherCitiesDictionary["R"]) != 0:
      findNearestCityAndSend(game, biggestCity, otherCitiesDictionary["R"])
      return
    elif len(otherCitiesDictionary["P"]) != 0:
      findNearestCityAndSend(game, biggestCity, otherCitiesDictionary["P"])
      return
  elif theCity.getProductionType() == "S":
    if len(otherCitiesDictionary["P"]) != 0:
      findNearestCityAndSend(game, biggestCity, otherCitiesDictionary["P"])
      return
    elif len(otherCitiesDictionary["S"]) != 0:
      findNearestCityAndSend(game, biggestCity, otherCitiesDictionary["S"])
      return
  else:
    return
  findNearestCityAndSend(game, biggestCity, otherCities)

  

def findNearestCityAndSend(game, fromCityID, cityList):
  nearestEnemy = 0
  minDistance = 99999999
  for aCity in cityList:
    newDist = game.numberOfTurnsBetweenCities(fromCityID, aCity.getCityID())
    if newDist < minDistance:
      minDistance = newDist
      nearestEnemy = aCity.getCityID()
  game.giveOrder(fromCityID, nearestEnemy, game.getCityDictionary()[fromCityID].getSoldiers())
